...
class TestServerMessengerProvider
...
describe("EndToEnd", () => {
  const [clientToServerStream, serverToClientStream] = getPipedStreams<
    string,
    string
  >();
  const clientMessenger = new ClientMessenger(
    new MessageStream(new JSONStream(clientToServerStream))
  );
  const serverMessenger = new ServerMessenger(
    new MessageStream(new JSONStream(serverToClientStream))
  );

...
class FakeServerMessengerProvider extends EventEmitter implements ServerMessengerProvider {
...
describe("DatabaseServer", () => {
    ...
    beforeEach(async () => {
        ...
        fakeMessengerProvider = new FakeServerMessengerProvider(new ServerMessenger(new MessageStream(fakeObjectStream)));
    ...
    context("connect request", () => {
        it("creates a local connection for a connect request", async () => {
            ...
            assert.deepEqual(await fakeObjectStream.getSentMessage(), { type: "connect", message: { code: RESPONSE_CODE.OK, value: DEFAULT_VALUE }});
    ...
    context("update request", () => {
        it("updates local database and returns updated value", async () => {
            ...
            fakeObjectStream.postMessage({ type: "update", message: { key: KEY, value: NEW_VALUE }});

            assert.deepEqual(await await fakeObjectStream.getSentMessage(), { type: "update", message: { code: RESPONSE_CODE.OK, value: NEW_VALUE }});
    ...
    context("local update", () => {
        it("notifies the client of the update", async () => {
            ...
            assert.deepEqual(await fakeObjectStream.getSentMessage(), { type: "remote_update", message: { key: KEY, value: NEW_VALUE }});
        });
    });
    ...
    context("disconnect request", () => {
        it("closes a record disconnection request", async () => {
            ...
            fakeObjectStream.postMessage({ type: "disconnect", message: { key: KEY }});

            assert.deepEqual(await fakeObjectStream.getSentMessage(), { type: "disconnect", message: { code: RESPONSE_CODE.OK }});

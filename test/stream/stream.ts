...
describe("JSONStream", () => {
  ...
  context("read", () => {
    ...
    it("splits multiple posted objects", async () => {
      await fakeStream.writeInput(
        JSON.stringify(MESSAGE) + "\n" + JSON.stringify(MESSAGE) + "\n"
      );

      const result = [await jsonStream.read(), await jsonStream.read()];
...
describe("MessageStream", () => {
  ...
  context("write", () => {
    it("writes messages", async () => {
      ...
      assert.deepEqual(await fakeStream.readOutput(), {
        type: "m1",
        message: MESSAGE_1,
      });
    });
  ...
  context("on", () => {
    it("sends an event to registered message listeners", async () => {
      const { promise, resolver } = await getPromiseResolver<
        TEST_MESSAGE_MAP[keyof TEST_MESSAGE_MAP]
      >();

...
export class Database<T> {
    ...
    onRecordChange(id: string, callback: (value?: Entry<T>) => void) {
        this.listeners.push(this.underlyingDatabase
            ...
            .on("change", change => callback(change.deleted ? undefined : change.doc)));
    ...
    onChange(callback: (id: string, value?: Entry<T>) => void) {
        this.listeners.push(this.underlyingDatabase
            ...
            .on("change", change => callback(change.id, change.deleted ? undefined : change.doc)));
    }
    ...
    async addRecord(value: NewEntry<T>): Promise<Entry<T>> {
        const response = value._id === undefined ? await this.underlyingDatabase.post(value) : await this.underlyingDatabase.put(value);
    ...
    async clear() {
        const records = await this.underlyingDatabase.allDocs({ include_docs: true });
        await this.underlyingDatabase.bulkDocs(records.rows.map(row => ({ ...(row.doc as T), _deleted: true }) ));
    }

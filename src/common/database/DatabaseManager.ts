...
export class DatabaseManager {
  ...
  async getRecord<T>(name: string, id: string, defaultProvider?: () => T) {
    return await this.withDatabase<T, T>(name, (database) =>
      database.getRecord(id, defaultProvider)
    );
  }

  async addRecord<T>(name: string, value: NewEntry<T>): Promise<Entry<T>> {
    return await this.withDatabase<T, Entry<T>>(name, (database) =>
      database.addRecord(value)
    );
  }

  async withDatabase<T, R = void>(
    name: string,
    operation: (database: Database<T>) => Promise<R>
  ): Promise<R> {
  ...
  close(database: Database<any>) {
    ...
    if (connections.length === 1) {
      ...
      setTimeout(
        () => this.closeDatabase(database.name),
        DATABASE_REUSE_DELAY_MS
      );
    } else {

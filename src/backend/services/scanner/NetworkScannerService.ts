...
import {
  ...
  NETWORK_SCAN_DATABASE as NETWORK_SCAN_DATABASE,
} from "../../../common/models/NetworkScan";
...
export class NetworkScannerService implements Service {
  static Builder: ServiceBuilder<NetworkScannerService> = {
    ...
    build: async (databaseService: DatabaseService) =>
      new NetworkScannerService(databaseService),
  };
  ...
  constructor(databaseService: DatabaseService) {
    ...
    this.scanRequestQueue = this.databaseManager.getDatabase<ScanRequest>(
      NETWORK_SCAN_DATABASE
    );
  }

  async start() {
    await this.databaseManager.getRecord(
      APPS_DATABASE,
      "network_scanner",
      () => ({
        _id: "network_scanner",
  ...
  private async handleScanRequest(request?: Entry<ScanRequest>) {
    ...
    await this.databaseManager.withDatabase<Device>(
      DEVICES_DATABASE,
      async (deviceDatabase) => {
        ...
        await this.scan(request, async (ip, mac) => {
          ...
          // Create a new device
          const newDevice = await deviceDatabase.addRecord({
  ...
  private async scan(
    request: Entry<ScanRequest>,
    deviceFoundCallback: (ip: string, mac: string) => Promise<void>
  ) {
    ...
    request = await this.scanRequestQueue.updateRecord({
      ...request,
      response,
    });

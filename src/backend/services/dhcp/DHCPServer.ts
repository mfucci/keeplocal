...
export class DHCPServer {
  ...
  constructor(
    ...
    this.settingDatabase =
      databaseManager.getDatabase<DHCPSettings>(SETTINGS_DATABASE);
    this.deviceDatabase = databaseManager.getDatabase<Device>(DEVICES_DATABASE);

    // TODO: listen for settings changes
    this.settingDatabase.onRecordChange(DHCPService.Builder.name, (settings) =>
  ...
  private async init() {
    ...
    if (!this.ipMap.has(myIp)) {
      ...
      await this.deviceDatabase.addRecord({
        ...createDevice(myMac, IpType.STATIC),
        name: "keeplocal",
        ip: myIp,
        lastSeen: Date.now(),
      });
  ...
  private async checkStaticDeviceRecord(ip: string, name: string) {
    ...
    if (mac === undefined)
      throw new Error(`Cannot find network equipment with IP ${ip}`);
    this.ipMap.set(ip, mac);
    await this.deviceDatabase.addRecord({
      ...createDevice(mac, IpType.STATIC),
      name,
      ip,
      lastSeen: Date.now(),
    });
  ...
  private async handleDiscover(request: Request) {
    ...
    this.messenger.sendOffer(
      request,
      this.settings,
      device.ip as string,
      this.getRouter(device.permissions?.internet)
    );
  }
  ...
  private getRouter(internetPermission: boolean = true) {
    return internetPermission
      ? this.settings.gateway_ip
      : this.settings.dhcp_ip;
  }

  private async getDevice(request: Request) {
    const { mac, hostname, classId } = request;
    const device = await this.deviceDatabase.getRecord(mac, () =>
      createDevice(mac, IpType.DYNAMIC, { internet: true })
    );
    if (device.ip === undefined) device.ip = await this.assignNewIp(mac);
    await this.deviceDatabase.updateRecord({
      ...device,
      lastSeen: Date.now(),
      hostname,
      classId,

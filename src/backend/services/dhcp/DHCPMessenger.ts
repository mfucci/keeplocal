...
import {
  ...
  Packet,
  ParameterListOption,
  BroadcastAddressOption,
  Socket,
  AddressRequestOption,
  HostnameOption,
  ClassIdOption,
  ...
  packet.options.forEach((option) => {
    switch (option.type) {
      case DHCPOptions.ParameterList:
        result.parameterRequestList = (option as ParameterListOption).value;
        break;
      case DHCPOptions.AddressRequest:
        result.requestedIp = (option as AddressRequestOption).value;
        break;
      case DHCPOptions.Hostname:
        result.hostname = (option as HostnameOption).value;
        break;
      case DHCPOptions.ClassId:
        result.classId = (option as ClassIdOption).value;
        break;
    }
  });
  ...
  request.parameterRequestList?.forEach((parameter) => {
    switch (parameter) {
      case DHCPOptions.SubnetMask:
        packet.options.push(new SubnetMaskOption(SUBNET_MASK));
        break;
      case DHCPOptions.DomainName:
        packet.options.push(new DomainNameOption(settings.local_domain));
        break;
      case DHCPOptions.DhcpServerId:
        packet.options.push(new DHCPServerIdOption(settings.dhcp_ip));
        break;
      case DHCPOptions.Gateways:
        packet.options.push(new GatewaysOption([routerIp]));
        break;
      case DHCPOptions.DomainServer:
        packet.options.push(new DomainServerOption([settings.dns_ip]));
        break;
      case DHCPOptions.BroadcastAddress:
        packet.options.push(
          new BroadcastAddressOption(
            ipUtil.subnet(settings.dhcp_ip, SUBNET_MASK).broadcastAddress
          )
        );
        break;
    }

...
const FAUXTON_BUNDLE_JS =
  "dashboard.assets/js/bundle-34997e32896293a1fa5d71f79eb1b4f7.js";
...
export class DatabaseService implements Service {
  static Builder: ServiceBuilder<DatabaseService> = {
    ...
    build: async (http: HTTPService) => {
      const settings = await new LocalDatabaseManager().getRecord(
        SETTINGS_DATABASE,
        NAME,
        () => DEFAULT_SETTINGS
      );
  ...
  async start() {
    ...
    server.use(
      `${baseUrlPath}${dataUrlPath}/_utils/${FAUXTON_BUNDLE_JS}`,
      (req, res) => res.send(this.patchFauxtonJs(baseUrlPath, dataUrlPath))
    );
    server.use(
      baseUrlPath + dataUrlPath,
      expressPouchDB(LocalPouchDb, { logPath, configPath })
    );

    await this.databaseManager.getRecord(APPS_DATABASE, "database", () => ({
      _id: "database",
      name: "Database",
      icon: "file_storage.png",
      type: AppType.External,
      url: this.getUiUrl(),
      groupId: INSTALLED_GROUP_ID,
      order: 0,
    }));
  ...
  private patchFauxtonJs(baseUrl: string, dataUrl: string) {
    ...
    const jsFile = fs
      .readFileSync(
        path.join(
          __dirname,
          `../../../../node_modules/pouchdb-fauxton/www/${FAUXTON_BUNDLE_JS}`
        )
      )
      .toString();
    return jsFile
      ...
      .replace(
        /window\.location\.origin\+\"\/_replicator/g,
        `window.location.origin+"${databaseUrl}/_replicator`
      )
      .replace(/url:\"\/_users/g, `url:"${databaseUrl}/_users`)
      .replace(
        'window.location.origin+"/"+o.default.utils.safeURLName',
        `window.location.origin+"${databaseUrl}/"+o.default.utils.safeURLName`
      );
  }
}

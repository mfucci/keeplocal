...
export class DNSMessenger extends EventEmitter {
  ...
  sendARecords({ name, query }: Request, records: RecordWithTtl[]) {
    records.forEach(({ address, ttl }) =>
      query.addAnswer(name, createARecord(address), ttl)
    );
  ...
  sendAAAARecords({ name, query }: Request, records: RecordWithTtl[]) {
    records.forEach(({ address, ttl }) =>
      query.addAnswer(name, createAaaaRecord(address), ttl)
    );
    this.server.send(query);
  }
  ...
  sendSoaRecord(
    { name, query }: Request,
    { nsname, hostmaster, serial, refresh, retry, expire, minttl }: SoaRecord
  ) {
    query.addAnswer(
      name,
      createSoaRecord({
  ...
  sendSrvRecords({ name, query }: Request, records: SrvRecord[]) {
    records.forEach(({ name: target, port, priority, weight }) =>
      query.addAnswer(name, createSrvRecord({ port, target, priority, weight }))
    );
    this.server.send(query);
  }

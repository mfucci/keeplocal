...
class CommandLineHandler {
    ...
    async listDevices() {
        const devices = await (await this.getDatabaseManager()).withDatabase<Device, Device[]>(DEVICES_DATABASE, database => database.getRecords());
        console.log(table([
            [
                "Name",
                "IP",
    ...
    startDaemon() {
        new Launcher().start(DaemonService.Builder)

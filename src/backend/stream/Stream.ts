...
export declare interface Stream<READ_T, WRITE_T = READ_T> {
  on<K extends keyof StreamEventMap>(
    event: K,
    listener: (...value: StreamEventMap[K]) => void
  ): this;
  once<K extends keyof StreamEventMap>(
    event: K,
    listener: (...value: StreamEventMap[K]) => void
  ): this;
  emit<K extends keyof StreamEventMap>(
    event: K,
    ...value: StreamEventMap[K]
  ): boolean;
}
...
export declare interface StreamProvider<READ_T, WRITE_T = READ_T> {
  on<K extends keyof StreamProviderEventMap<READ_T, WRITE_T>>(
    event: K,
    listener: (...value: StreamProviderEventMap<READ_T, WRITE_T>[K][]) => void
  ): this;
  once<K extends keyof StreamProviderEventMap<READ_T, WRITE_T>>(
    event: K,
    listener: (...value: StreamProviderEventMap<READ_T, WRITE_T>[K][]) => void
  ): this;
  emit<K extends keyof StreamProviderEventMap<READ_T, WRITE_T>>(
    event: K,
    ...value: StreamProviderEventMap<READ_T, WRITE_T>[K]
  ): boolean;
}

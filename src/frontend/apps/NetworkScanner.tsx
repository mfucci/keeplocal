  ...
  render() {
    return (
      <Grid container spacing={3}>
        <SectionCard>
          Network scanner using ping / ARP to discover all the devices on your
          network.
        </SectionCard>
...
export class NetworkScannerCard extends React.Component<Props, State> {
  ...
  private async startScan() {
    ...
    const request = await databaseManager.addRecord<ScanRequest>(
      NETWORK_SCAN_DATABASE,
      {}
    );
  ...
  render() {
    ...
    return (
      <SectionCard>
        ...
        <Record<ScanRequest> dbName={NETWORK_SCAN_DATABASE} id={requestId}>
          {(request) => (
            <Lifecycle
              onUnmount={() => this.setState({ requestId: undefined })}
            >
              <IfDefined value={request.response}>
                {(response) => (
                  <React.Fragment>
                    <LinearProgressWithLabel
                      value={(response.ipsScanned * 100) / response.ipsToScan}
                    />
                  </React.Fragment>
                )}
              </IfDefined>

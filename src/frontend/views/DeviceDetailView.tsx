        ...
        return (
            <Record<Device> dbName={DEVICES_DATABASE} id={id}>{(device, update, remove) => 
                <Grid container spacing={3} columns={{ xs: 6, md: 12 }}>
                    <Grid item xs={12}>
                        <Card>
                            <With value={device}>{({ name, category, vendor, model, groupId }) =>
                                <CardContent>
                            ...
                            <With value={device}>{({ mac, ip = "<unassigned>", ipType, hostname, lastSeen, classId }) =>
                                <Now>{now =>
                                    <CardContent>
                                        ...
                                        <div>IP address: {ip} ({ipType})</div>
                                        <div>Status: {isOnline(now, device) ? "online" : "offline"} (last seen: {lastSeen ? <TimeAgo now={now} timestamp={lastSeen} /> : "never"})</div>
                                        <If condition={hostname !== undefined}><div>Hostname: {hostname}</div></If>
                                        <If condition={classId !== undefined}><div>Class ID: {classId}</div></If>
                                    </CardContent>
                                }</Now>
                            }</With>
                        </Card>

  ...
  render() {
    ...
    return (
      <Grid container spacing={3}>
        <Records<Device> dbName={DEVICES_DATABASE}>
          {(devices) => (
            <Now>
              {(now) => (
                <SectionCard>
                  {devices.length} devices,{" "}
                  {devices.filter((device) => isOnline(now, device)).length}{" "}
                  online
                </SectionCard>
              )}
            </Now>
        ...
        <SectionCard>
          <Button
            ...
            onClick={() => this.setState({ editOrder: !editOrder })}
          >
            {editOrder ? "Done" : "Reorder"}
          </Button>
        </SectionCard>

        <Record<App> dbName={APPS_DATABASE} id="network_scanner">
          {(networkScanner) => <NetworkScannerCard />}
        </Record>
      </Grid>
    );
  }

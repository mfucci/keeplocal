...
export class GroupController<T extends GroupItem> extends React.Component<
  Props<T>,
  State
> {
  ...
  async removeGroup(id: string) {
    const { databaseManager, groupsDb, itemsDb } = {
      ...this.context,
      ...this.state,
      ...this.props,
    };
    ...
    await databaseManager.withDatabase<T>(itemsDb, async (database) => {
      ...
      const unassignedItems = items
        .filter((item) => item.groupId === UNASSIGNED_GROUP_ID)
        .sort(sortByOrder);
    ...
    await databaseManager.withDatabase<Group>(groupsDb, (database) =>
      database.remove(id)
    );
  }

  async addGroup(name: string): Promise<string> {
    const { databaseManager, groupsDb } = {
      ...this.context,
      ...this.state,
      ...this.props,
    };
